...
dxl_comm_result, dxl_error = packetHandler.write4ByteTxRx(portHandler, 11, 112, 50)
dxl_comm_result, dxl_error = packetHandler.write4ByteTxRx(portHandler, 11, ADDR_GOAL_POSITION, 453)
dxl_comm_result, dxl_error = packetHandler.write1ByteTxRx(portHandler, 12, ADDR_TORQUE_ENABLE, TORQUE_ENABLE)
dxl_comm_result, dxl_error = packetHandler.write4ByteTxRx(portHandler, 12, 112, 50)
...
dxl_comm_result, dxl_error = packetHandler.write4ByteTxRx(portHandler, 14, 112, 50)
dxl_comm_result, dxl_error = packetHandler.write4ByteTxRx(portHandler, 14, ADDR_GOAL_POSITION, 2355)
dxl_comm_result, dxl_error = packetHandler.write1ByteTxRx(portHandler, 16, ADDR_TORQUE_ENABLE, TORQUE_ENABLE)
dxl_comm_result, dxl_error = packetHandler.write4ByteTxRx(portHandler, 16, 112, 50)
dxl_comm_result, dxl_error = packetHandler.write4ByteTxRx(portHandler, 16, ADDR_GOAL_POSITION, 2296)
dxl_comm_result, dxl_error = packetHandler.write1ByteTxRx(portHandler, 15, ADDR_TORQUE_ENABLE, TORQUE_ENABLE)
dxl_comm_result, dxl_error = packetHandler.write4ByteTxRx(portHandler, 15, 112, 50)
dxl_comm_result, dxl_error = packetHandler.write4ByteTxRx(portHandler, 15, ADDR_GOAL_POSITION, 3029)

# Wait for upward movement to complete (adjust delay as needed)
import time
time.sleep(2)  # Delay to allow the arm to rise completely

dxl_comm_result, dxl_error = packetHandler.write1ByteTxRx(portHandler, 13, ADDR_TORQUE_ENABLE, TORQUE_ENABLE)
dxl_comm_result, dxl_error = packetHandler.write4ByteTxRx(portHandler, 13, 112, 50)
dxl_comm_result, dxl_error = packetHandler.write4ByteTxRx(portHandler, 13, ADDR_GOAL_POSITION, 2301)

# Enable Dynamixel Torque for waving motor
dxl_comm_result, dxl_error = packetHandler.write1ByteTxRx(portHandler, wave_motor, ADDR_TORQUE_ENABLE, TORQUE_ENABLE)
if dxl_comm_result != COMM_SUCCESS:
    print("Failed to enable torque on waving motor:", packetHandler.getTxRxResult(dxl_comm_result))
elif dxl_error != 0:
...
for cycle in range(wave_cycles):
    for position in wave_positions:
        # Send command to move waving motor to goal position
        dxl_comm_result, dxl_error = packetHandler.write4ByteTxRx(portHandler, wave_motor, ADDR_GOAL_POSITION, position)
        if dxl_comm_result != COMM_SUCCESS:
            print("Error communicating with motor:", packetHandler.getTxRxResult(dxl_comm_result))
        elif dxl_error != 0:
            print("Motor error:", packetHandler.getRxPacketError(dxl_error))
        time.sleep(wave_delay)  # Delay between wave movements
# Enable Dynamixel Torque
